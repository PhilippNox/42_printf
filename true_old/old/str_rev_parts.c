...
/*   Updated: 2019/05/20 18:43:37 by wgorold          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void test1()
{
	...
	print_arr(list, 24);
	if (list[0] == STOP_SHORT)
		return ;
	power2(&start, list[0]);
	print_t_str(&start); print_t_str_human(&start); ft_putchar('\n');

	idx = 0;
	while (list[++idx] != STOP_SHORT)
	{
		power2(&add, list[idx]);
		print_t_str(&add); print_t_str_human(&add); ft_putchar('\n');
		sum_t_str(&reduce, &start, &add);
		print_t_str(&reduce); print_t_str_human(&reduce); ft_putchar('\n');
		start = reduce;
	}
}
